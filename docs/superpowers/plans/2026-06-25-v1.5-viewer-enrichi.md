# v1.5 « Viewer enrichi » — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
>
> Numérotation : initialement planifié comme « v1.4 », renumérotée en **v1.5**
> le 2026-09-06 (le coffre de mots de passe a mergé dans `main` en premier et
> en devient la vraie v1.4 chronologique).
...
**Tech Stack:** Python 3.10+ stdlib uniquement (`re`, `html`, `posixpath`, `urllib.parse`, `http.server`). Tests via `unittest` (`pytest` n'est pas installé). Spec : `docs/superpowers/specs/2026-06-25-v1.5-viewer-enrichi-design.md`.
...
**Ce qui s'est réellement passé (2026-09-06)** : le coffre de mots de passe a
mergé dans `main` en premier et a pris le numéro 1.4.0 pendant que cette
branche était en pause. Après avoir rattrapé `main` (`git merge main`), ce
Task 5 a donc été exécuté avec **1.5.0** au lieu de 1.4.0 partout (fichiers,
message de commit, entrée CHANGELOG placée au-dessus de celle du coffre) —
cf. la note de numérotation en tête de ce document et de la spec associée.
